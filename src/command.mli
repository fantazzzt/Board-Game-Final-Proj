...
(** The type [deploy_command] represents a player command that is decomposed
...
val parse_deploy : string -> deploy_command
...
(** [parse_names str] parses a player's input into a list of names, as follows.
       Each word (i.e., consecutive sequence of non-space characters)
       of [str] becomes a separate name (i.e. element) in the reutrned list.
       Requires: [str] contains exactly three words.
       Raises: [Empty] if [str] is the empty string or contains only
       spaces.
       Raises: [Malformed] if the command is malformed. A command is
       {i malformed} if there are words, but not exactly three *)
